...
class GpsOffScreen extends StatelessWidget {
  const GpsOffScreen({Key? key, this.gpsEnabled, this.permission})
    : super(key: key);
    ...
    return Scaffold(
      body: Container(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.location_off_rounded, size: 80),
            const SizedBox(height: 3),
            const Text('Oops !', style: TextStyles.textBold16),
            const SizedBox(height: 15),
            const Text('Gps est Désactivé', style: TextStyles.text),
            const SizedBox(height: 15),
            MaterialButton(
              onPressed: () async {

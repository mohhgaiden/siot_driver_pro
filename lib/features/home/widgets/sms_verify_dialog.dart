...
import 'package:siot_driver_pro/features/home/pdf/services/ems_pdf_service.dart';
...
class _SMSVerifyDialogState extends State<SMSVerifyDialog> {
  ...
  void refresh() {
    final data =
        stats.keys
            .where(
              (element) =>
                  stats.get(element)["uuid_user"] == widget.uuid &&
                  stats.get(element)["MacAddrs"] == widget.macAddrs &&
                  stats.get(element)["InfoDate"] >
                      initialDay.millisecondsSinceEpoch &&
                  stats.get(element)["InfoDate"] <
                      (initialDay.millisecondsSinceEpoch + 86400000),
            )
            .map((key) {
  ...
  int selectedIndex = 2;
  late DateTime initialDay = DateTime(
    DateTime.now().year,
    DateTime.now().month,
    DateTime.now().day,
  );
  ...
  void initState() {
    seconds = int.parse(
      Hive.box('LOGGED_IN_USER').getAt(0)["user_clear_interval"],
    );
    refresh();
        ...
        Stack(
          children: <Widget>[
            Container(
              ...
              child: const Text('Impression', style: TextStyles.textBold16),
            ),
            Positioned(
              top: 0.0,
              right: 0.0,
        ...
        MyButton(
          ...
          onPressed: () async {
                ...
                for (int i = 0; i < items.length; i++)
                  InvoiceItem(
                    ...
                    date: DateTime.fromMillisecondsSinceEpoch(
                      items.elementAt(i)["InfoDate"],
                    ).toString().substring(0, 19),
                    temperature: items.elementAt(i)["temperature"],
                    humidity:
                        widget.type == '1' ? items.elementAt(i)["humidity"] : 0,
                    pression:
                        widget.type == '1' ? items.elementAt(i)["presure"] : 0,
                  ),
              ],
            );
            ...
            if (invoice.items.isNotEmpty) {
              emsPdfService.savePdfFile("emsPdf", data);
            }
            if (invoice.items.isEmpty) {
              showDialog(
                ...
                builder:
                    (context) => AlertDialog(
                      content: const Text(
                        "Aucune information pour le jour sélectionné",
                      ),
                      actions: [
                        TextButton(
                          onPressed: () {

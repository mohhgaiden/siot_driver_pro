...
=======
  ...
  void addSensorRead(int i) async {
    var response = await http.post(
      Uri.parse(
        "https://admin.sirius-iot.eu/Mobile/API/SiotDriver2022/Android/sensor_read.php",
      ),
      body: {
        "uuid_user": Hive.box('SENSOR_READ').getAt(i)["uuid_user"],
>>>>>>> edc460f (Initial commit)
  ...
  Type3SensorRead(ScanResult result) async {
    for (int i = 0; i < Hive.box('LIST_CAPTEURS').length; i++) {
      if (result.device.remoteId.str ==
              Hive.box('LIST_CAPTEURS').getAt(i)['MacAddrs'] &&
          Hive.box('LIST_CAPTEURS').getAt(i)['Type'] == '3') {
        var manufacturerData = Uint8List.fromList(
          result.advertisementData.serviceData[Guid("2a6e")]!,
        );
        var temp = ByteData.sublistView(
          manufacturerData,
          0,
          2,
        ).getUint16(0, Endian.little).toRadixString(2);
        if (temp.length < 16) {
        ...
        if (items
                .where(
                  (element) =>
                      element["uuid_user"] ==
                          Hive.box('LOGGED_IN_USER').getAt(0)["uuid_user"] &&
                      element["MacAddrs"] == result.device.remoteId.str,
                )
                .isEmpty ||
            items
                    .where(
...
=======
          ...
          if ((itemsAlert.elementAt(
                        itemsAlert.indexWhere(
                          (item) =>
                              item["MacAddrs"] == result.device.remoteId.str,
                        ),
                      )["checkedtemperature"] ==
                      '1' &&
                  (double.parse(temp) <
                          itemsAlert.elementAt(
                            itemsAlert.indexWhere(
                              (item) =>
                                  item["MacAddrs"] ==
            ...
            NotificationService.showSimpleNotification(
              3,
              "SIOT Driver",
              "Problèmes dans le capteur '${Hive.box('LIST_CAPTEURS').get(i)["Name"]}'.",
            );
            setState(() {
              hide = false;
            });
  ...
  Type1SensorRead(ScanResult result) async {
    for (int i = 0; i < Hive.box('LIST_CAPTEURS').length; i++) {
      if (result.device.remoteId.str ==
              Hive.box('LIST_CAPTEURS').getAt(i)['MacAddrs'] &&
          Hive.box('LIST_CAPTEURS').getAt(i)['Type'] == '1') {
        var manufacturerData = Uint8List.fromList(
          result.advertisementData.manufacturerData[1177]!,
        );
        var temp = ByteData.sublistView(
          manufacturerData,
          1,
          3,
        ).getUint16(0, Endian.big).toRadixString(2);
        var hum = (ByteData.sublistView(
                  manufacturerData,
                  3,
                  5,
                ...
                400)
            .toStringAsFixed(2);
        var press = ((ByteData.sublistView(
                      manufacturerData,
                      5,
                      7,
                    ).getUint16(0, Endian.big) +
...
=======
          ...
          if ((itemsAlert.elementAt(
            ...
            NotificationService.showSimpleNotification(
              1,
              "SIOT Driver",
              /*"Problèmes dans le capteur '${Hive.box('LIST_CAPTEURS').get(i)["Name"]}'."*/ "${result.timeStamp}",
            );
            setState(() {
              hide = false;
            });
  ...
  Type5SensorRead(ScanResult result) async {
    for (int i = 0; i < Hive.box('LIST_CAPTEURS').length; i++) {
      if (result.device.remoteId.str ==
              Hive.box('LIST_CAPTEURS').getAt(i)['MacAddrs'] &&
          Hive.box('LIST_CAPTEURS').getAt(i)['Type'] == '5') {
        var manufacturerData = Uint8List.fromList(
          result.advertisementData.manufacturerData[65535]!,
        );
        var temp = (ByteData.sublistView(
          manufacturerData,
          7,
        ...
        var light = ((ByteData.sublistView(
                  manufacturerData,
                  11,
                  13,
...
=======
  ...
  Type6SensorRead(ScanResult result) async {
    for (int i = 0; i < Hive.box('LIST_CAPTEURS').length; i++) {
      if (result.device.remoteId.str ==
              Hive.box('LIST_CAPTEURS').getAt(i)['MacAddrs'] &&
          Hive.box('LIST_CAPTEURS').getAt(i)['Type'] == '6') {
        var manufacturerData = Uint8List.fromList(
          result.advertisementData.manufacturerData[65535]!,
        );
        var temp = (ByteData.sublistView(
...
=======
  ...
  Type10SensorRead(ScanResult result) async {
    for (int i = 0; i < Hive.box('LIST_CAPTEURS').length; i++) {
      if (result.device.remoteId.str ==
              Hive.box('LIST_CAPTEURS').getAt(i)['MacAddrs'] &&
          Hive.box('LIST_CAPTEURS').getAt(i)['Type'] == '10') {
        var manufacturerData = Uint8List.fromList(
          result.advertisementData.manufacturerData[3278]!,
        );
        var hum = (((manufacturerData[16] << 8) | manufacturerData[17]) / 100.0)
            .toStringAsFixed(2);
        var temp = (((manufacturerData[14] << 8) | manufacturerData[15]) /
                100.0)
            .toStringAsFixed(2);
        var vol = (((manufacturerData[11] << 8) | manufacturerData[12]) /
                1000.0)
            .toStringAsFixed(2);
        if (items
                .where(
...
=======
  ...
  @override
  void initState() {
    if (Hive.box('ACTIVITY_START_END').isNotEmpty &&
        Hive.box('ACTIVITY_START_END').values.last["activity_type"] == '1') {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        showDialog(
          context: context,
          builder:
                    ...
                    onPressed: () async {
                      await Hive.box('ACTIVITY_START_END').add({
                        "uuid_user":
                            Hive.box('LOGGED_IN_USER').getAt(0)["uuid_user"],
                        "activity_type": '2',
                        "activity_date_heur": DateTime.now()
                            .toString()
                            .substring(0, 19),
                        "gps_lat": lat,
                        "gps_long": long,
...
=======
    timer3 = Timer.periodic(
      Duration(
        seconds:
            int.parse(
              Hive.box('LOGGED_IN_USER').getAt(0)["user_store_interval"],
            ) *
            60,
      ),
      (timer) {
  ...
  Future<void> checkConnect() async {
    ...
    if (result) {
      print('connected');
      if (result && Hive.box('SENSOR_READ').isNotEmpty) {
        addSensorRead(0);
      }
      if (result && Hive.box('ACTIVITY_START_END').isNotEmpty) {
        addEnd(0);
      }
    }
  }
...
=======
              ...
              icon: Icon(
                ...
                color: !isStart ? Colors.black : Colors.grey,
                size: 24,
              ),
            ),
            IconButton(
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(
                            '${Hive.box('LIST_CAPTEURS').length} capteurs affecter',
                          ...
                          Column(
                            children: [
                              for (int i = 0; i < scanResults.length; i++)
                                for (
                                  int j = 0;
                                  j < Hive.box('LIST_CAPTEURS').length;
                                  j++
                                )
                                  if (scanResults[i].device.remoteId.str ==
                                      Hive.box(
                                        'LIST_CAPTEURS',
                                      ).get(j)["MacAddrs"])
                                    Column(
                                        ...
                                        if (Hive.box(
                                          ...
                                          ScanResults(
                                            ...
                                            param:
                                                Hive.box(
                                                  'LOGGED_IN_USER',
                                                ).getAt(0)["user_can_param"],
                                            print:
                                                Hive.box(
                                                  'LOGGED_IN_USER',
                                                ).getAt(0)["user_can_print"],
                                            report:
                                                Hive.box(
                                                  'LOGGED_IN_USER',
                                                ).getAt(0)["user_can_report"],
                                            name:
                                                Hive.box(
                                                  'LIST_CAPTEURS',
                                                ).get(j)["Name"],
